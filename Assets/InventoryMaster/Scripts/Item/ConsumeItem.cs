    ...
    public void OnPointerDown(PointerEventData data)
    {
        if (this.gameObject.transform.parent.parent.parent.GetComponent<EquipmentSystem>() == null)
        {
            ...
            if (data.button == PointerEventData.InputButton.Right)
            {
                ...
                {
                    bool check = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerInventory>().inventory.GetComponent<Inventory>().CheckIfItemAlreadyExist(item.itemID, item.itemValue);
                    ...
                    CraftSystem cS = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerInventory>().craftSystemGO.GetComponent<CraftSystem>();
                    cS.deleteItems(item);
                    CraftResultSlot result = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerInventory>().craftSystemGO.transform.GetChild(3).GetComponent<CraftResultSlot>();
                    result.temp = 0;
